...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-06-2023 a las 03:17:47
...
DROP PROCEDURE IF EXISTS `spUpdatePlan`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spUpdatePlan` (IN `_COD_PLAN` INT(10), IN `_NOMBRE` VARCHAR(100), IN `_DESCRIPCION` LONGTEXT, IN `_TELEFONO` LONGTEXT)  BEGIN
UPDATE plan SET NOMBRE = _NOMBRE, DESCRIPCION = _DESCRIPCION, TELEFONO = _TELEFONO WHERE COD_PLAN = _COD_PLAN;
END$$
...
(11, 'PLAN DURISIMO', 'MUY DIFICILISIMO', '555233', 1),
